...
stream_env_stats <- function(
  ...
  if (length(hist_col) != 1L) {
    stop(
      "Could not uniquely identify historical column for base indicator = '", base_ind, "'. ",
...
for (i in 1:n.CUs) {
  ...
  fwTrate_i <- stream_env_stats(
    ...
    {
      d <- .
      bind_rows(d, compute_gcm_quantiles_long(d))
    } %>%
    dplyr::distinct(
  ...
  fw_all_i <- fw_all_i %>%
    ...
    mutate(
      gcm_name = coalesce(gcm_name.y, gcm_name.x),
      .after = gcm
    ) %>%
