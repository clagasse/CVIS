...
#1) calculate models means 
BCCM_SST_mean <- mean(BCCM_SST_sub$BCCM_SST_45AnnAvg, na.rm=TRUE)
BCCM_SSS_mean <- mean(BCCM_SSS_sub$BCCM_SSS_45AnnAvg, na.rm=TRUE)
BCCM_SSPH_mean<- mean(BCCM_SSPH_sub$BCCM_SSPH_45AnnAvg, na.rm=TRUE)
BCCM_SST_sd   <- sd(BCCM_SST_sub$BCCM_SST_45AnnAvg, na.rm=TRUE)
BCCM_SSS_sd   <- sd(BCCM_SSS_sub$BCCM_SSS_45AnnAvg, na.rm=TRUE)
BCCM_SSPH_sd  <- sd(BCCM_SSPH_sub$BCCM_SSPH_45AnnAvg, na.rm=TRUE)         

NEP_SST_mean <- mean(NEP_SST_sub$NEP_SST_45AnnAvg, na.rm=TRUE)
NEP_SSS_mean <- mean(NEP_SSS_sub$NEP_SSS_45AnnAvg, na.rm=TRUE)
NEP_SSPH_mean<- mean(NEP_SSPH_sub$NEP_SSPH_45AnnAvg, na.rm=TRUE)
NEP_SST_sd   <- sd(NEP_SST_sub$NEP_SST_45AnnAvg, na.rm=TRUE)
NEP_SSS_sd   <- sd(NEP_SSS_sub$NEP_SSS_45AnnAvg, na.rm=TRUE)
NEP_SSPH_sd  <- sd(NEP_SSPH_sub$NEP_SSPH_45AnnAvg, na.rm=TRUE)  

SSC_SST_mean <- mean(SSC_SST_sub$SSC_SST_45AnnAvg, na.rm=TRUE)
SSC_SSS_mean <- mean(SSC_SSS_sub$SSC_SSS_45AnnAvg, na.rm=TRUE)
SSC_SST_sd   <- sd(SSC_SST_sub$SSC_SST_45AnnAvg, na.rm=TRUE)
SSC_SSS_sd   <- sd(SSC_SSS_sub$SSC_SSS_45AnnAvg, na.rm=TRUE)
 
# Calculate z score for each MAZ
SST_AnnAvg$BCCM_zscore<-(SST_AnnAvg$BCCM- BCCM_SST_mean)/BCCM_SST_sd
SST_AnnAvg$NEP_zscore<-(SST_AnnAvg$NEP - NEP_SST_mean)/NEP_SST_sd
SST_AnnAvg$SSC_zscore<-(SST_AnnAvg$SSC - SSC_SST_mean)/SSC_SST_sd

SSS_AnnAvg$BCCM_zscore<-(SSS_AnnAvg$BCCM- BCCM_SSS_mean)/BCCM_SSS_sd
SSS_AnnAvg$NEP_zscore<-(SSS_AnnAvg$NEP - NEP_SSS_mean)/NEP_SSS_sd
SSS_AnnAvg$SSC_zscore<-(SSS_AnnAvg$SSC - SSC_SSS_mean)/SSC_SSS_sd

SSPH_AnnAvg$BCCM_zscore<-(SSPH_AnnAvg$BCCM- BCCM_SSPH_mean)/BCCM_SSPH_sd
SSPH_AnnAvg$NEP_zscore<-(SSPH_AnnAvg$NEP - NEP_SSPH_mean)/NEP_SSPH_sd
...
colAnAvgZsc_all<-c('MAZ', 'BCCM Mean','NEP Mean','SSC Mean','BCCM Z score','NEP Z score', 'SSC Z score')
colAnAvgZsc_2<- c('MAZ', 'BCCM Mean','NEP Mean','BCCM Z score','NEP Z score')
...
#remove variable no longer needed 
rm(BCCM_SST_mean,BCCM_SSS_mean,BCCM_SSPH_mean,BCCM_SST_sd,BCCM_SSS_sd,
   BCCM_SSPH_sd,NEP_SST_mean,NEP_SSS_mean ,NEP_SSPH_mean,NEP_SST_sd,
   NEP_SSS_sd,NEP_SSPH_sd,SSC_SST_mean ,SSC_SSS_mean,SSC_SST_sd,SSC_SSS_sd,
