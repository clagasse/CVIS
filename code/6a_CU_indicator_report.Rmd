...
#indicator dataframe
cu_std <- all_flat_std[all_flat_std$FULL_CU_IN == cu_i,]
...
fw_sp_cu <- fw_sp_ind[stream_cu_sub,] %>%
  ...
  mutate(model_rs = if_any(starts_with("keep_habitat"), ~ . == TRUE)) %>%#get boolean for model spawning and rearing
...
cu_all <- cu_all_raw %>%
  ...
  mutate(
    # Convert NaN strings to actual NA (if they slipped in)
...
tbl_view <- cu_all %>%
  ...
  select(any_of(c(
    ...
    # species/all means (end of table)
    "sp_mean", "allcu_mean"
  )))

# Rename columns (short, descriptive)
tbl_view <- tbl_view %>%
  rename(
...
# Create subset table for demographic indicators
create_indicator_reactable(tbl_view, c("CUstatus", "CUnmat"))
```
...
create_indicator_reactable(tbl_view, c("lowQpdelta", "highQpdelta"))
```
...
create_indicator_reactable(tbl_view, c("CT"))
```
...
create_indicator_reactable(tbl_view, c("migrT", "migrQ", "migrdist"))
```
...
plot_title = paste0("Migration path length = ", cu_std$migrdist[1], "km") 
)
...
p2 <- marine_indicator_plot(filter(CImpact_points, MAZ_Acrony == cu_mar$MAZ[1]),
                      MAZ_sp = MAZ,
